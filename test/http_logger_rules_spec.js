...
describe('HttpLogger', () => {
  ...
  it('uses copy_session_field rules', () => {
    ...
    let queue = [];
    let logger = new HttpLogger({ queue, rules: 'copy_session_field /.*/' });
    ...
    queue = [];
    logger = new HttpLogger({ queue, rules: 'copy_session_field /session_id/' });
    HttpMessage.send(logger, request, helper.mockResponseWithHtml(), helper.MOCK_HTML, helper.MOCK_JSON);
    expect(queue.length).to.equal(1);
    ...
    queue = [];
    logger = new HttpLogger({ queue, rules: 'copy_session_field /blah/' });
    HttpMessage.send(logger, request, helper.mockResponseWithHtml(), helper.MOCK_HTML, helper.MOCK_JSON);
    expect(queue.length).to.equal(1);
    expect(queue[0]).not.to.contain('["session_field:');

    queue = [];
    logger = new HttpLogger({ queue, rules: 'copy_session_field /butterfly/\ncopy_session_field /session_id/' });
    HttpMessage.send(logger, request, helper.mockResponseWithHtml(), helper.MOCK_HTML, helper.MOCK_JSON);
    expect(queue.length).to.equal(1);
    expect(queue[0]).to.contain('["session_field:butterfly","poison"]');
    expect(queue[0]).to.contain('["session_field:session_id","asdf1234"]');
  ...
  it('uses copy_session_field and remove rules', () => {
    ...
    queue = [];
    logger = new HttpLogger({ queue, rules: 'copy_session_field !.*!\n!session_field:butterfly! remove' });
    HttpMessage.send(logger, request, helper.mockResponseWithHtml(), helper.MOCK_HTML, helper.MOCK_JSON);
    expect(queue.length).to.equal(1);
    expect(queue[0]).not.to.contain('["session_field:butterfly",');
    expect(queue[0]).to.contain('["session_field:session_id","asdf1234"]');
  ...
  it('uses remove rules', () => {
    ...
    queue = [];
    logger = new HttpLogger({ queue, rules: '!response_body! remove' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, helper.MOCK_JSON);
    expect(queue.length).to.equal(1);
    ...
    queue = [];
    logger = new HttpLogger({ queue, rules: '!request_body|response_body! remove' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, helper.MOCK_JSON);
    expect(queue.length).to.equal(1);
    expect(queue[0]).not.to.contain('["request_body",');
    expect(queue[0]).not.to.contain('["response_body",');
    ...
    queue = [];
    logger = new HttpLogger({ queue, rules: '!request_header:.*! remove' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, helper.MOCK_JSON);
    expect(queue.length).to.equal(1);
    expect(queue[0]).to.contain('["request_body",');
    ...
    queue = [];
    logger = new HttpLogger({ queue, rules: '!request_header:abc! remove\n!response_body! remove' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, helper.MOCK_JSON);
    expect(queue.length).to.equal(1);
    expect(queue[0]).to.contain('["request_body",');
  ...
  it('uses remove_if rules', () => {
    let queue = [];
    let logger = new HttpLogger({ queue, rules: '!response_header:blahblahblah! remove_if !.*!' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, helper.MOCK_JSON);
    expect(queue.length).to.equal(1);

    queue = [];
    logger = new HttpLogger({ queue, rules: '!.*! remove_if !.*!' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, helper.MOCK_JSON);
    expect(queue.length).to.equal(0);

    queue = [];
    logger = new HttpLogger({ queue, rules: '!request_body! remove_if !.*!' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, helper.MOCK_JSON);
    expect(queue.length).to.equal(1);
    expect(queue[0]).not.to.contain('["request_body",');
    expect(queue[0]).to.contain('["response_body",');
    ...
    queue = [];
    logger = new HttpLogger({ queue, rules: '!response_body! remove_if !.*!' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, helper.MOCK_JSON);
    expect(queue.length).to.equal(1);
    expect(queue[0]).to.contain('["request_body",');
    expect(queue[0]).not.to.contain('["response_body",');
    ...
    queue = [];
    logger = new HttpLogger({ queue, rules: '!response_body|request_body! remove_if !.*World.*!' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, helper.MOCK_JSON);
    expect(queue.length).to.equal(1);
    expect(queue[0]).to.contain('["request_body",');
    expect(queue[0]).not.to.contain('["response_body",');
    ...
    queue = [];
    logger = new HttpLogger({ queue, rules: '!response_body|request_body! remove_if !.*blahblahblah.*!' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, helper.MOCK_JSON);
    expect(queue.length).to.equal(1);
    expect(queue[0]).to.contain('["request_body",');
    expect(queue[0]).to.contain('["response_body",');
  ...
  it('uses remove_if_found rules', () => {
    let queue = [];
    let logger = new HttpLogger({ queue, rules: '!response_header:blahblahblah! remove_if_found !.*!' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, helper.MOCK_JSON);
    expect(queue.length).to.equal(1);

    queue = [];
    logger = new HttpLogger({ queue, rules: '!.*! remove_if_found !.*!' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, helper.MOCK_JSON);
    expect(queue.length).to.equal(0);

    queue = [];
    logger = new HttpLogger({ queue, rules: '!request_body! remove_if_found !.*!' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, helper.MOCK_JSON);
    expect(queue.length).to.equal(1);
    expect(queue[0]).not.to.contain('["request_body",');
    expect(queue[0]).to.contain('["response_body",');
    ...
    queue = [];
    logger = new HttpLogger({ queue, rules: '!response_body! remove_if_found !.*!' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, helper.MOCK_JSON);
    expect(queue.length).to.equal(1);
    expect(queue[0]).to.contain('["request_body",');
    expect(queue[0]).not.to.contain('["response_body",');
    ...
    queue = [];
    logger = new HttpLogger({ queue, rules: '!response_body|request_body! remove_if_found !World!' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, helper.MOCK_JSON);
    expect(queue.length).to.equal(1);
    expect(queue[0]).to.contain('["request_body",');
    expect(queue[0]).not.to.contain('["response_body",');
    ...
    queue = [];
    logger = new HttpLogger({ queue, rules: '!response_body|request_body! remove_if_found !blahblahblah!' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, helper.MOCK_JSON);
    expect(queue.length).to.equal(1);
    expect(queue[0]).to.contain('["request_body",');
    expect(queue[0]).to.contain('["response_body",');
  ...
  it('uses remove_unless rules', () => {
    let queue = [];
    let logger = new HttpLogger({ queue, rules: '!response_header:blahblahblah! remove_unless !.*!' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, helper.MOCK_JSON);
    expect(queue.length).to.equal(1);

    queue = [];
    logger = new HttpLogger({ queue, rules: '!.*! remove_unless !.*blahblahblah.*!' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, helper.MOCK_JSON);
    expect(queue.length).to.equal(0);

    queue = [];
    logger = new HttpLogger({ queue, rules: '!request_body! remove_unless !.*blahblahblah.*!' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, helper.MOCK_JSON);
    expect(queue.length).to.equal(1);
    expect(queue[0]).not.to.contain('["request_body",');
    expect(queue[0]).to.contain('["response_body",');
    ...
    queue = [];
    logger = new HttpLogger({ queue, rules: '!response_body! remove_unless !.*blahblahblah.*!' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, helper.MOCK_JSON);
    expect(queue.length).to.equal(1);
    expect(queue[0]).to.contain('["request_body",');
    expect(queue[0]).not.to.contain('["response_body",');
    ...
    queue = [];
    logger = new HttpLogger({ queue, rules: '!response_body|request_body! remove_unless !.*World.*!' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, helper.MOCK_JSON);
    expect(queue.length).to.equal(1);
    expect(queue[0]).not.to.contain('["request_body",');
    expect(queue[0]).to.contain('["response_body",');
    ...
    queue = [];
    logger = new HttpLogger({ queue, rules: '!response_body|request_body! remove_unless !.*!' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, helper.MOCK_JSON);
    expect(queue.length).to.equal(1);
    expect(queue[0]).to.contain('["request_body",');
    expect(queue[0]).to.contain('["response_body",');
    ...
    queue = [];
    logger = new HttpLogger({ queue, rules: '!response_body! remove_unless !.*!\n!request_body! remove_unless !.*!' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, helper.MOCK_JSON);
    expect(queue.length).to.equal(1);
    expect(queue[0]).to.contain('["request_body",');
    expect(queue[0]).to.contain('["response_body",');
  ...
  it('uses remove_unless_found rules', () => {
    let queue = [];
    let logger = new HttpLogger({ queue, rules: '!response_header:blahblahblah! remove_unless_found !.*!' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, helper.MOCK_JSON);
    expect(queue.length).to.equal(1);

    queue = [];
    logger = new HttpLogger({ queue, rules: '!.*! remove_unless_found !blahblahblah!' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, helper.MOCK_JSON);
    expect(queue.length).to.equal(0);

    queue = [];
    logger = new HttpLogger({ queue, rules: '!request_body! remove_unless_found !blahblahblah!' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, helper.MOCK_JSON);
    expect(queue.length).to.equal(1);
    expect(queue[0]).not.to.contain('["request_body",');
    expect(queue[0]).to.contain('["response_body",');
    ...
    queue = [];
    logger = new HttpLogger({ queue, rules: '!response_body! remove_unless_found !blahblahblah!' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, helper.MOCK_JSON);
    expect(queue.length).to.equal(1);
    expect(queue[0]).to.contain('["request_body",');
    expect(queue[0]).not.to.contain('["response_body",');
    ...
    queue = [];
    logger = new HttpLogger({ queue, rules: '!response_body|request_body! remove_unless_found !World!' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, helper.MOCK_JSON);
    expect(queue.length).to.equal(1);
    expect(queue[0]).not.to.contain('["request_body",');
    expect(queue[0]).to.contain('["response_body",');
    ...
    queue = [];
    logger = new HttpLogger({ queue, rules: '!response_body|request_body! remove_unless_found !.*!' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, helper.MOCK_JSON);
    expect(queue.length).to.equal(1);
    expect(queue[0]).to.contain('["request_body",');
    expect(queue[0]).to.contain('["response_body",');
  ...
  it('uses replace rules', () => {
    let queue = [];
    let logger = new HttpLogger({ queue, rules: '!response_body! replace !blahblahblah!, !ZZZZZ!' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, helper.MOCK_JSON);
    expect(queue.length).to.equal(1);
    ...
    queue = [];
    logger = new HttpLogger({ queue, rules: '!response_body! replace !World!, !Mundo!' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, helper.MOCK_JSON);
    expect(queue.length).to.equal(1);
    expect(queue[0]).to.contain('["response_body","<html>Hello Mundo!</html>"],');

    queue = [];
    logger = new HttpLogger({ queue, rules: '!request_body|response_body! replace !^.*!, !ZZZZZ!' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, helper.MOCK_JSON);
    expect(queue.length).to.equal(1);
    ...
    queue = [];
    logger = new HttpLogger({ queue, rules: '!request_body! replace !^.*!, !QQ!\n!response_body! replace !^.*!, !SS!' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, helper.MOCK_JSON);
    expect(queue.length).to.equal(1);
    ...
    queue = [];
    logger = new HttpLogger({ queue, rules: '!response_body! replace !World!, !!' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, helper.MOCK_JSON);
    expect(queue.length).to.equal(1);
    expect(queue[0]).to.contain('["response_body","<html>Hello !</html>"],');

    queue = [];
    logger = new HttpLogger({ queue, rules: '!response_body! replace !.*!, !!' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, helper.MOCK_JSON);
    expect(queue.length).to.equal(1);
    expect(queue[0]).not.to.contain('["response_body",');

    queue = [];
    logger = new HttpLogger({ queue, rules: '!response_body! replace !World!, !Z!' });
  ...
  it('uses replace rules with complex expressions', () => {
    ...
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML.replace('World', 'rob@resurface.io'), helper.MOCK_JSON);
    expect(queue.length).to.equal(1);
    expect(queue[0]).to.contain('["response_body","<html>Hello x@y.com!</html>"],');

    queue = [];
    logger = new HttpLogger({ queue, rules: `/response_body/ replace /[0-9\\.\\-\\/]{9,}/, /xyxy/` });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML.replace('World', '123-45-1343'), helper.MOCK_JSON);
    expect(queue.length).to.equal(1);
    expect(queue[0]).to.contain('["response_body","<html>Hello xyxy!</html>"],');

    queue = [];
    logger = new HttpLogger({ queue, rules: '!response_body! replace !World!, !<b>$&</b>!' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, helper.MOCK_JSON);
    expect(queue.length).to.equal(1);
    expect(queue[0]).to.contain('["response_body","<html>Hello <b>World</b>!</html>"],');

    queue = [];
    logger = new HttpLogger({ queue, rules: '!response_body! replace !(World)!, !<b>$1</b>!' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, helper.MOCK_JSON);
    expect(queue.length).to.equal(1);
    expect(queue[0]).to.contain('["response_body","<html>Hello <b>World</b>!</html>"],');
  ...
  it('uses sample rules', () => {
    ...
    try {
      new HttpLogger({ queue, rules: 'sample 10\nsample 99' });
    ...
    const logger = new HttpLogger({ queue, rules: 'sample 10' });
    for (let i = 1; i <= 100; i++) {
  ...
  it('uses stop rules', () => {
    let queue = [];
    let logger = new HttpLogger({ queue, rules: '!response_header:blahblahblah! stop' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, null);
    expect(queue.length).to.equal(1);

    queue = [];
    logger = new HttpLogger({ queue, rules: '!.*! stop' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, helper.MOCK_JSON);
    expect(queue.length).to.equal(0);

    queue = [];
    logger = new HttpLogger({ queue, rules: '!request_body! stop' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), null, helper.MOCK_JSON);
    expect(queue.length).to.equal(0);

    queue = [];
    logger = new HttpLogger({ queue, rules: '!response_body! stop' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, null);
    expect(queue.length).to.equal(0);
  ...
  it('uses stop_if rules', () => {
    let queue = [];
    let logger = new HttpLogger({ queue, rules: '!response_header:blahblahblah! stop_if !.*!' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, null);
    expect(queue.length).to.equal(1);

    queue = [];
    logger = new HttpLogger({ queue, rules: '!response_body! stop_if !.*!' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, helper.MOCK_JSON);
    expect(queue.length).to.equal(0);

    queue = [];
    logger = new HttpLogger({ queue, rules: '!response_body! stop_if !.*World.*!' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, helper.MOCK_JSON);
    expect(queue.length).to.equal(0);

    queue = [];
    logger = new HttpLogger({ queue, rules: '!response_body! stop_if !.*blahblahblah.*!' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, helper.MOCK_JSON);
    expect(queue.length).to.equal(1);
  });
  ...
  it('uses stop_if_found rules', () => {
    let queue = [];
    let logger = new HttpLogger({ queue, rules: '!response_header:blahblahblah! stop_if_found !.*!' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, null);
    expect(queue.length).to.equal(1);

    queue = [];
    logger = new HttpLogger({ queue, rules: '!response_body! stop_if_found !.*!' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, helper.MOCK_JSON);
    expect(queue.length).to.equal(0);

    queue = [];
    logger = new HttpLogger({ queue, rules: '!response_body! stop_if_found !World!' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, helper.MOCK_JSON);
    expect(queue.length).to.equal(0);
    ...
    queue = [];
    logger = new HttpLogger({ queue, rules: '!response_body! stop_if_found !blahblahblah!' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, helper.MOCK_JSON);
    expect(queue.length).to.equal(1);
  });
  ...
  it('uses stop_unless rules', () => {
    let queue = [];
    let logger = new HttpLogger({ queue, rules: '!response_header:blahblahblah! stop_unless !.*!' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, null);
    expect(queue.length).to.equal(0);

    queue = [];
    logger = new HttpLogger({ queue, rules: '!response_body! stop_unless !.*!' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, null);
    expect(queue.length).to.equal(1);

    queue = [];
    logger = new HttpLogger({ queue, rules: '!response_body! stop_unless !.*World.*!' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, null);
    expect(queue.length).to.equal(1);

    queue = [];
    logger = new HttpLogger({ queue, rules: '!response_body! stop_unless !.*blahblahblah.*!' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, null);
    expect(queue.length).to.equal(0);
  });
  ...
  it('uses stop_unless_found rules', () => {
    let queue = [];
    let logger = new HttpLogger({ queue, rules: '!response_header:blahblahblah! stop_unless_found !.*!' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, null);
    expect(queue.length).to.equal(0);

    queue = [];
    logger = new HttpLogger({ queue, rules: '!response_body! stop_unless_found !.*!' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, null);
    expect(queue.length).to.equal(1);

    queue = [];
    logger = new HttpLogger({ queue, rules: '!response_body! stop_unless_found !World!' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, null);
    expect(queue.length).to.equal(1);
    ...
    queue = [];
    logger = new HttpLogger({ queue, rules: '!response_body! stop_unless_found !blahblahblah!' });
    HttpMessage.send(logger, helper.mockRequestWithJson2(), helper.mockResponseWithHtml(), helper.MOCK_HTML, null);
    expect(queue.length).to.equal(0);
  });
});
