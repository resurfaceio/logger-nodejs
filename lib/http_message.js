...
class HttpMessage {
   ...
   */
  static send(logger, request, response, response_body = undefined, request_body = undefined, now = undefined, interval = undefined, custom_fields = undefined) {
    ...
    return logger.submit_if_passing(message, custom_fields);
  }
  ...
  static appendRequestHeaders(message, request) {
    ...
    if (headers) {
      for (const key of Object.keys(headers)) {
        ...
        if (!addressInHeaders) {
          if (lowerKey === 'x-forwarded-for' || lowerKey === 'forwarded-for' || lowerKey === 'forwarded' || lowerKey === 'cf-connecting-ip' || lowerKey === 'fastly-client-ip' || lowerKey === 'true-client-ip') {
            addressInHeaders = true;
          } else if (lowerKey === 'x-forwarded' || lowerKey === 'x-client-ip' || lowerKey === 'x-real-ip' || lowerKey === 'x-cluster-client-ip') {
            message.push([`request_header: x-forwarded-for ${key.toLowerCase()}`, headers[key]]);
            addressInHeaders = true;
          }
        }
      }
      if (!addressInHeaders && request.remote_addr !== '') {
        message.push(['request_header: x-forwarded-for', request.remote_addr]);
      }
    }
  }
