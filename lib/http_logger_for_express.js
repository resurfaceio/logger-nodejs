            ...
            if (headers != undefined) {
                if (HttpLogger.isStringContentType(headers['content-type'])) {
                    if (body instanceof Buffer) {
                        HttpMessage.send(this._logger, request, response, body.toString());
                    } else if (typeof body === 'string' || body instanceof String) {
                        HttpMessage.send(this._logger, request, response, body);
                    } else {
                        HttpMessage.send(this._logger, request, response, JSON.stringify(body));
                    }
                }
            }
        }
