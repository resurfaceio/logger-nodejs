...
class HttpRules {
    ...
    static parse(rules) {
        ...
        if ((typeof rules !== 'undefined') && (rules !== null)) {
            ...
            rules = rules.replace(/^\s*include strict\s*$/gm, this.strictRules);
    ...
    constructor(rules) {
        ...
        if (rules_exist) {
            this.rules = rules.replace(/^\s*include default\s*$/gm, HttpRules.defaultRules);
            if (rules.trim().length === 0) this.rules = HttpRules.defaultRules;
        } else {
            this.rules = HttpRules.defaultRules;
        }

        // parse and break out rules by verb
        let prs = HttpRules.parse(this.rules);
        this.rules_allow_http_url = prs.filter(r => 'allow_http_url' === r.verb).length > 0;
        this.rules_copy_session_field = prs.filter(r => 'copy_session_field' === r.verb);
        this.rules_remove = prs.filter(r => 'remove' === r.verb);
        this.rules_remove_if = prs.filter(r => 'remove_if' === r.verb);
        this.rules_remove_if_found = prs.filter(r => 'remove_if_found' === r.verb);
        this.rules_remove_unless = prs.filter(r => 'remove_unless' === r.verb);
        this.rules_remove_unless_found = prs.filter(r => 'remove_unless_found' === r.verb);
        this.rules_replace = prs.filter(r => 'replace' === r.verb);
        this.rules_sample = prs.filter(r => 'sample' === r.verb);
        this.rules_stop = prs.filter(r => 'stop' === r.verb);
        this.rules_stop_if = prs.filter(r => 'stop_if' === r.verb);
        this.rules_stop_if_found = prs.filter(r => 'stop_if_found' === r.verb);
        this.rules_stop_unless = prs.filter(r => 'stop_unless' === r.verb);
        this.rules_stop_unless_found = prs.filter(r => 'stop_unless_found' === r.verb);
        ...
        // validate rules
        if (this.rules_sample.length > 1) throw new EvalError('Multiple sample rules');

        // mark immutable properties
        Object.defineProperty(this, 'rules', {configurable: false, writable: false});
        Object.defineProperty(this, 'rules_allow_http_url', {configurable: false, writable: false});
        Object.defineProperty(this, 'rules_copy_session_field', {configurable: false, writable: false});
        Object.defineProperty(this, 'rules_remove', {configurable: false, writable: false});
        Object.defineProperty(this, 'rules_remove_if', {configurable: false, writable: false});
        Object.defineProperty(this, 'rules_remove_if_found', {configurable: false, writable: false});
        Object.defineProperty(this, 'rules_remove_unless', {configurable: false, writable: false});
        Object.defineProperty(this, 'rules_remove_unless_found', {configurable: false, writable: false});
        Object.defineProperty(this, 'rules_replace', {configurable: false, writable: false});
        Object.defineProperty(this, 'rules_sample', {configurable: false, writable: false});
        Object.defineProperty(this, 'rules_skip_compression', {configurable: false, writable: false});
        Object.defineProperty(this, 'rules_skip_submission', {configurable: false, writable: false});
        Object.defineProperty(this, 'rules_stop', {configurable: false, writable: false});
        Object.defineProperty(this, 'rules_stop_if', {configurable: false, writable: false});
        Object.defineProperty(this, 'rules_stop_if_found', {configurable: false, writable: false});
        Object.defineProperty(this, 'rules_stop_unless', {configurable: false, writable: false});
        Object.defineProperty(this, 'rules_stop_unless_found', {configurable: false, writable: false});
    }
    ...
    apply(details) {
        ...
        for (let r of this.rules_stop)
            for (let d of details)
                if (d[0].match(r.scope)) return null;
        for (let r of this.rules_stop_if_found)
            for (let d of details)
                if (d[0].match(r.scope) && d[1].match(r.param1)) return null;
        for (let r of this.rules_stop_if)
            for (let d of details)
                if (d[0].match(r.scope) && d[1].match(r.param1)) return null;
        let passed = 0;
        for (let r of this.rules_stop_unless_found)
            for (let d of details)
                if (d[0].match(r.scope) && d[1].match(r.param1)) passed++;
        if (passed !== this.rules_stop_unless_found.length) return null;
        passed = 0;
        for (let r of this.rules_stop_unless)
            for (let d of details)
                if (d[0].match(r.scope) && d[1].match(r.param1)) passed++;
        if (passed !== this.rules_stop_unless.length) return null;

        // do sampling if configured
        if ((this.rules_sample.length === 1) && (Math.random() * 100 >= this.rules_sample[0].param1)) return null;

        // winnow sensitive details based on remove rules if configured
        for (let r of this.rules_remove)
            details = details.filter(d => !d[0].match(r.scope));
        for (let r of this.rules_remove_unless_found)
            details = details.filter(d => !d[0].match(r.scope) || d[1].match(r.param1));
        for (let r of this.rules_remove_if_found)
            details = details.filter(d => !d[0].match(r.scope) || !d[1].match(r.param1));
        for (let r of this.rules_remove_unless)
            details = details.filter(d => !d[0].match(r.scope) || d[1].match(r.param1));
        for (let r of this.rules_remove_if)
            details = details.filter(d => !d[0].match(r.scope) || !d[1].match(r.param1));
        if (details.length === 0) return null;

        // mask sensitive details based on replace rules if configured
        for (let r of this.rules_replace)
            for (let d of details)
