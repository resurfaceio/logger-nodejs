...
class HttpRules {
  ...
  apply(details) {
    ...
    for (const r of this.stop) for (const d of details) if (d[0].match(r.scope)) return null;
    for (const r of this.stop_if_found) for (const d of details) if (d[0].match(r.scope) && d[1].match(r.param1)) return null;
    for (const r of this.stop_if) for (const d of details) if (d[0].match(r.scope) && d[1].match(r.param1)) return null;
    let passed = 0;
    for (const r of this.stop_unless_found) for (const d of details) if (d[0].match(r.scope) && d[1].match(r.param1)) passed++;
    if (passed !== this.stop_unless_found.length) return null;
    passed = 0;
    for (const r of this.stop_unless) for (const d of details) if (d[0].match(r.scope) && d[1].match(r.param1)) passed++;
    ...
    for (const r of this.remove) details = details.filter((d) => !d[0].match(r.scope));
    for (const r of this.remove_unless_found) details = details.filter((d) => !d[0].match(r.scope) || d[1].match(r.param1));
    for (const r of this.remove_if_found) details = details.filter((d) => !d[0].match(r.scope) || !d[1].match(r.param1));
    for (const r of this.remove_unless) details = details.filter((d) => !d[0].match(r.scope) || d[1].match(r.param1));
    for (const r of this.remove_if) details = details.filter((d) => !d[0].match(r.scope) || !d[1].match(r.param1));
    if (details.length === 0) return null;

    // mask sensitive details based on replace rules if configured
    for (const r of this.replace) for (const d of details) if (d[0].match(r.scope)) d[1] = d[1].replace(r.param1, r.param2);
